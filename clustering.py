...
__all__ = ['PIC', 'Kmeans', 'DBSCAN', 'cluster_assign', 'arrange_clustering']
...
def run_dbscan(x):
    ...
    """
    # faiss implementation of k-means
    dbscan = cumlDBSCAN(eps=1, min_samples=15)
...
class DBSCAN(object):
    ...
    def cluster(self, data):
        ...
        # cluster the data
        labels, loss = run_dbscan(xb)
        way = len(set(labels))
        self.images_lists = [[] for i in range(way)]
        for i in range(len(data)):
